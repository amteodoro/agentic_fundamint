...
export function MarketCapSharesChart({ ticker }: { ticker: string }) {
  ...
  const marketCapFormatter = useMemo(() =>
  ...
  const convertedData = useMemo(() => {
    if (currency === 'EUR' && exchangeRate) {
      return data.map(d => ({
        ...d,
        "Market Cap": d["Market Cap"] ? d["Market Cap"] * exchangeRate : undefined
      }));
    }
    return data;
  ...
  if (loading) return (
    <Card>
      <CardHeader>
        <CardTitle>Market Cap & Shares ({currency})</CardTitle>
      </CardHeader>
      <CardContent>
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={300}>
          <ComposedChart data={convertedData}>
            ...
            <XAxis dataKey="Date" />
            <YAxis yAxisId="left" tickFormatter={marketCapFormatter}>
              <Label value={`Market Cap (${currency})`} angle={-90} position="insideLeft" style={{ textAnchor: 'middle' }} />
            </YAxis>
