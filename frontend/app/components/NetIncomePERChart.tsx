...
export function NetIncomePERChart({ ticker }: { ticker: string }) {
    ...
    <Card>
      ...
      <CardContent>
        <div className="h-[300px] w-full flex items-center justify-center">
          <p>Loading Net Income & PER chart...</p>
        </div>
      </CardContent>
    </Card>
  );
  ...
  if (!data.length) return (
    <Card>
      ...
      <CardContent>
        <div className="h-[300px] w-full flex items-center justify-center">
          <p>No data available for Net Income & PER chart.</p>
        </div>
      </CardContent>
    </Card>
  );
        ...
        <ResponsiveContainer width="100%" height={300}>
          <ComposedChart data={data}>
            ...
            <YAxis yAxisId="left" tickFormatter={(tick) => formatNumber(tick, 0)}>
              <Label value="Annual Net Income" angle={-90} position="insideLeft" style={{ textAnchor: 'middle' }} />
            </YAxis>
            <YAxis yAxisId="right" orientation="right" tickFormatter={(tick) => formatNumber(tick, 1)}>
              <Label value="PER" angle={-90} position="insideRight" style={{ textAnchor: 'middle' }} />
            </YAxis>
            <Tooltip content={<CustomTooltip />} />
            <Legend />
            <Bar yAxisId="left" dataKey="Annual Net Income" fill={COLORS[0]} />
            <Line yAxisId="right" type="monotone" dataKey="PER" stroke={COLORS[1]} />
          </ComposedChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>
